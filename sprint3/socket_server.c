// fixes windows vsc complaning imports
#ifdef _WIN32
#include <winsock2.h>
#include <ws2tcpip.h>
...
int main()
{
	...
	while (1)
	{
		...
		PacketFloat packetFromClient = receivePacketFloat(client_socket);
		// if (packetFromClient.payload == NULL)
		// {
		// 	perror("receive packet failed");
		// 	exit(EXIT_FAILURE);
		// }
		printf("packet from client: (header: (version: %u, type: %u, length: %u), payload: %f)\n",
